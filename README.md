...
У каждого пользователя может быть своя **роль** из библиотеки промптов — готовые сценарии (ассистент, репетитор, копирайтер и др.), которые задают системный промпт для модели.
...
- `📚 Промпты` открывает библиотеку готовых ролей. Выбор роли меняет поведение бота и сбрасывает историю диалога.
- `🧹 Очистить диалог` очищает историю текущего пользователя, но не сбрасывает выбранную модель и роль.
...
Готовые роли хранятся в `prompts/prompts.json`. Каждый промпт описывает роль, контекст и задачу для модели; поле `response_format` необязательно.

Примеры встроенных ролей: стандартный ассистент, фитнес-тренер, копирайтер, репетитор, планировщик путешествий и другие.

Чтобы добавить или изменить роль, отредактируйте JSON и перезапустите бота. Выбранная роль сохраняется для каждого пользователя в `data/context.json` отдельно от истории сообщений.
...
Выбранная модель и роль из библиотеки промптов хранятся отдельно для каждого пользователя и сохраняются в `data/context.json`.
